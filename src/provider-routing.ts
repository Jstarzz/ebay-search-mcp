...
export const providerDefinitions: Record<ProviderId, ProviderDefinition> = {
    "ebay-official": {
        ...
        recurringMonthlyAllowance: null,
        sharedAllowanceGroup: null,
        preserveForAmazon: false,
        envKeys: ["EBAY_CLIENT_ID", "EBAY_CLIENT_SECRET"],
        notes: "Primary eBay Browse API path.",
    },
    "ebay-selfhosted": {
        id: "ebay-selfhosted",
        ...
        sharedAllowanceGroup: null,
        preserveForAmazon: false,
        envKeys: ["EBAY_SELFHOSTED_URL"],
        notes: "Overflow/fallback eBay scraper endpoint.",
    },
    "bright-data": {
        id: "bright-data",
        providerClass: "recurring-free",
        recurringMonthlyAllowance: 5000,
        sharedAllowanceGroup: "bright-data",
        preserveForAmazon: true,
        envKeys: ["BRIGHT_DATA_API_KEY"],
        notes: "Shared pool; preserve capacity for Amazon unless cheaper AliExpress paths fail.",
    },
    scrapingdog: {
        id: "scrapingdog",
        providerClass: "recurring-free",
        recurringMonthlyAllowance: 200,
        sharedAllowanceGroup: null,
        preserveForAmazon: false,
        envKeys: ["SCRAPINGDOG_API_KEY"],
        notes: "Dedicated Amazon search/product API.",
    },
    hasdata: {
        id: "hasdata",
        providerClass: "recurring-free",
        recurringMonthlyAllowance: 1000,
        sharedAllowanceGroup: "hasdata",
        preserveForAmazon: true,
        envKeys: ["HASDATA_API_KEY"],
        notes: "Shared pool; preserve capacity for Amazon when possible.",
    },
    apify: {
        id: "apify",
        providerClass: "recurring-free",
        recurringMonthlyAllowance: null,
        sharedAllowanceGroup: "apify",
        preserveForAmazon: false,
        envKeys: ["APIFY_TOKEN"],
        notes: "$5 monthly platform-credit path; actor IDs are configured separately.",
    },
    serpapi: {
        id: "serpapi",
        providerClass: "recurring-free",
        recurringMonthlyAllowance: 250,
        sharedAllowanceGroup: null,
        preserveForAmazon: false,
        envKeys: ["SERPAPI_API_KEY"],
        notes: "Amazon discovery/search fallback rather than a full product-detail source.",
    },
    "amazon-selfhosted": {
        id: "amazon-selfhosted",
        providerClass: "self-hosted",
        recurringMonthlyAllowance: null,
        sharedAllowanceGroup: null,
        preserveForAmazon: false,
        envKeys: ["AMAZON_SELFHOSTED_URL"],
        notes: "Overflow/fallback Amazon scraper endpoint.",
    },
    "aliexpress-official": {
        id: "aliexpress-official",
        providerClass: "official",
        recurringMonthlyAllowance: null,
        sharedAllowanceGroup: null,
        preserveForAmazon: false,
        envKeys: ["ALIEXPRESS_APP_KEY", "ALIEXPRESS_APP_SECRET"],
        notes: "AliExpress Affiliate API when the account is approved and credentials work.",
    },
    "aliexpress-selfhosted": {
        id: "aliexpress-selfhosted",
        providerClass: "self-hosted",
        recurringMonthlyAllowance: null,
        sharedAllowanceGroup: null,
        preserveForAmazon: false,
        envKeys: ["ALIEXPRESS_SELFHOSTED_URL"],
        notes: "Cheap local fallback before consuming shared anti-bot capacity.",
    },
...
export function isProviderConfigured(provider: ProviderId, env: ProviderEnvironment = process.env): boolean {
    return providerDefinitions[provider].envKeys.every((key) => Boolean(env[key]?.trim()));
}

export function getConfiguredRoute(store: Store, env: ProviderEnvironment = process.env): ProviderId[] {
    return storeRoutes[store].filter((provider) => isProviderConfigured(provider, env));
}
